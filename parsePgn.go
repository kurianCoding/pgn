...
func Parse(pgnbytes []byte) *PGN {
	...
	for _, val := range movslic {
		...
		if len(move) == 4 {
			play = append(play, Move{White: move[1], Black: move[2]})
		} else {
			log.Println(len(move))
		}

	}
	return &PGN{Meta: getmeta, Moves: play}
}

func (p *PGN) String() string {
	var movstring string
	for _, mov := range p.Moves {
		movstring = fmt.Sprintf("%s%s%s", movstring, mov.White, mov.Black)
	}
	return movstring
}

func GetPiece(move string) string {
	return ""
}
